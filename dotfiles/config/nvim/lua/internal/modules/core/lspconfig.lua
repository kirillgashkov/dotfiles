return {
	...
	config = function(_, opts)
		...
		for server, server_opts in pairs(opts.x_servers) do
			server_opts = vim.tbl_deep_extend("force", default_server_opts, server_opts)

			if server == "efm" then
				local tools_from_language = {}
				for _, lt in ipairs(server_opts.x_tools) do
					local language, tool = lt[1], lt[2]
				...
				server_opts = vim.tbl_deep_extend("force", server_opts, {
					filetypes = vim.tbl_keys(tools_from_language),
					settings = {
						languages = tools_from_language,
					},
				})
			end
			...
			if server_opts.x_on_attach ~= nil then
				...
				server_opts = vim.tbl_deep_extend("force", server_opts, {
					on_attach = function(client, bufnr)
						...
						local handler = x_on_attach[ft] or x_on_attach["*"] or function(_, _) end
